...
import { KpiCard } from '../composants/KpiCard';
  ...
  return (
        ...
        <div className="flex items-center gap-1 rounded-xl bg-cream-100 p-1">
          {PERIODS.map((item) => (
            <button
              ...
              className={[
                'cursor-pointer rounded-lg px-3.5 py-1.5 text-[12.5px] font-bold transition',
                period === item.id ? 'bg-primary text-white shadow-primary-glow' : 'text-ink-muted hover:bg-cream-50',
              ].join(' ')}
            >
              {item.label}
            </button>
    ...
    >
      {period === 'day' ? (
        day ? (
          <div className="flex flex-col gap-5">
            <div className="grid grid-cols-4 gap-4">
              <KpiCard tone="primary" label="Collecté aujourd'hui" value={million(day.collectedToday)} hint={`+${day.collectedTodayDeltaPct}% vs hier`} />
              <KpiCard label="Reversé & rapproché" value={`${day.reconciledRate}%`} hint={`${day.reconciledCount}/${day.reconciledTotal} bordereaux`} />
              <KpiCard label="Contestations" value={String(disputesCount)} hint={`${resolvedDisputesCount} résolues · ${openDisputesCount} ouvertes`} />
              <KpiCard tone="danger" label="Écarts de caisse" value={fcfa(day.cashGapAmount)} hint={`${day.cashGapRejections} rejet${day.cashGapRejections > 1 ? 's' : ''}`} />
            </div>
        ...
        <div className="flex flex-col gap-5">
          <div className="grid grid-cols-4 gap-4">
            <KpiCard tone="primary" label={`Collecté en ${month.monthLabel.split(' ')[0].toLowerCase()}`} value={million(month.collectedMonth)} hint={`+${month.collectedMonthDeltaPct}% vs mois précédent`} />
            <KpiCard label="Taux de reversement" value={`${month.settlementRate.toString().replace('.', ',')}%`} />
            <KpiCard label="Régularité moyenne" value={`${averageRegularityRate}%`} hint="tous clients" />
            <KpiCard label="Nouveaux clients" value={`+${newClientsThisMonth}`} hint="mois en cours" />
          </div>
